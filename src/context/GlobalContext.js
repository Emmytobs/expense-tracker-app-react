...
export function GlobalProvider(props) {
    ...
    const deleteTransaction = async (transactionId) => {
        try {
            const response = await axios.delete(`https://emmytobs-expense-tracker-api.herokuapp.com/transaction/${transactionId}`, {
              headers: {
    ...
    const addTransaction = async (transactionForm) => {
        ...
        try {
          const response = await axios.post('https://emmytobs-expense-tracker-api.herokuapp.com/transaction', newTransaction, {
            headers: {
              'Authorization': `Bearer ${user.token}`
            }
          })
